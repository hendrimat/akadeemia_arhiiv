...
from alpha_shapes import create_boundary_polygons, smooth_polygon
from fonts import (
    query_google_fonts,
)
from medoids import medoid
from selection_handlers import SelectionHandlerBase
...
_DECKGL_HTML_TEMPLATE_STR = (_TEMPLATE_DIR / "kaart_html_template.html").read_text(
    encoding="utf-8"
)

_DECKGL_SCRIPT_TEMPLATE_STR = (_TEMPLATE_DIR / "kaart_script_template.html").read_text(
    encoding="utf-8"
)

_DECKGL_STYLE_TEMPLATE_STR = (_TEMPLATE_DIR / "kaart_style_template.html").read_text(
    encoding="utf-8"
)
...
def _get_js_dependency_sources(
    ...
    dict
        ...
        source content.
    """
    static_dir = Path(__file__).resolve().parent.parent / "web" / "static" / "js"
    js_dependencies = [
        "mobile_view.js",
        "datamap.js",
        "data_selection_manager.js"
    ]
    js_dependencies_src = {}
...
def _get_css_dependency_sources(
    ...
    dict
        ...
        source content.
    """
    static_dir = Path(__file__).resolve().parent.parent / "web" / "static" / "css"
    css_dependencies = ["containers_and_stacks.css", "style.css"]
...
def per_layer_cluster_colormaps(label_layers, label_color_map, n_swatches=5):
    ...
    for i, layer in enumerate(label_layers[::-1]):
        ...
        colormap_metadata = {
            "field": f"layer_{i}",
            "description": f"{descriptors[i]} KLASTRID",
            "colors": color_sample
            + [
...
def render_html(
    ...
    # Render the individual parts
    html_template = jinja2.Template(_DECKGL_HTML_TEMPLATE_STR)
    script_template = jinja2.Template(_DECKGL_SCRIPT_TEMPLATE_STR)
    style_template = jinja2.Template(_DECKGL_STYLE_TEMPLATE_STR)

    script_str = script_template.render(**render_vars)
    style_str = style_template.render(**render_vars)

    # Inject the script and style into the main body template
