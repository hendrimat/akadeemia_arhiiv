...
# --- Configuration ---
ARTICLES_JSON_PATH = '../data/numbers.json'
MODEL_NAME = 'all-mpnet-base-v2'
EMBEDDINGS_PATH = '../data/article_embeddings_all-mpnet-base-v2-flask.pt'
...
def get_recommendations_for_article(article_id):
    ...
    if embedding_idx is None:
        return jsonify({"error": "Article ID not found or does not have a description for recommendations"}), 404
    ...
    for score, doc_idx in zip(top_results[0], top_results[1]):
        ...
        recommendations.append({
            ...
            'file_path': rec_article['file_path']  # Pass the direct path for linking
        })
...
def search_articles_endpoint():
    ...
    top_k = 10  # Or get from query param: int(request.args.get('k', 10))
